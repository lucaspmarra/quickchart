...
function addColorsPlugin(chart) {
  if (chart.options?.plugins?.colorschemes) {
    return;
  }
...
function patternDraw(shapeType, backgroundColor, patternColor, requestedSize) {
  return function doPatternDraw() {
    ...
    global.document = {
      createElement: () => {
        return canvas.createCanvas(backgroundImageURL, size, size);
      },
    };
...
async function renderChartJs(
  ...
  console.log('renderChartJs options:', {
    width,
    height,
    backgroundColor,
    devicePixelRatio,
  ...
  //console.log('Data object:', untrustedChart.data);
  let chart;
  ...
  console.log('charts.js: ', chart.options);
  ...
  if (
    ...
    if (!chart.options.scales) {
      // TODO: Merge default options with provided options
      chart.options.scales = {
        yAxes: [
          {
            ticks: {
  ...
  Chart.plugins.register({
    id: 'backgroundImage',
    beforeDraw: async function(chartInstance) {
      const backgroundImageURL = chartInstance.options.plugins.backgroundImageURL;
      if (backgroundImageURL) {
        ...
        try {
          ...
          console.log('Loaded background image:', image);
          ctx.drawImage(image, 0, 0, chartInstance.chart.width, chartInstance.chart.height);
        } catch (error) {
  ...
  if (format === 'svg') {
    // SVG rendering doesn't work asynchronously.
